...
async function connectToWhatsapp() {
  ...
  function handleConnectionUpdate(update) {
    ...
    if (connection === "close") {
      ...
      console.log(
        `Koneksi terputus karena ${lastDisconnect.error}, Hubungkan kembali! ${reconnecting}`
      );
      if (reconnecting) {
        connectToWhatsapp();
      }
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    if (type === "notify" && !messages[0].key.fromMe) {
      try {
        ...
        const checkBan = isMessageGroup
          ? banPeople.includes(messages[0].key.participant.split("@")[0])
          : false;
        if (checkBan) {
        } else {
          ...
          if (chat.toLowerCase() === "ping") {
            ...
            const message2 =
              message1.extendedTextMessage.contextInfo.quotedMessage;
          ...
          if (isMessageGroup) {
            if (
              chat.includes("/ban") &&
              messages[0].key.participant == "6289612792131@s.whatsapp.net"
            ) {
              try {
                ...
                await sock.sendMessage(
                  number,
                  {
                    text: `Berhasil! ${ambilRequestChat} telah di ban untuk menggunakan fitur bot!"`,
                    mentions: [
                      ambilRequestChat.split("@")[1] + "@s.whatsapp.net",
                    ],
                  },
                  { quoted: messages[0] },
                  2000
              ...
              await sock.sendMessage(
                number,
                {
                  text: "Ogah ah, fitur ini hanya bisa di pakai untuk orang tampan @6289612792131",
                  mentions: ["6289612792131@s.whatsapp.net"],
                },
                { quoted: messages[0] }
              );
            }
            if (
              ...
              banPeople = banPeople.filter(
                (items) => items !== ambilRequestChat.split("@")[1]
              );
              ...
              await sock.sendMessage(
                number,
                {
                  text: `Berhasil! ${ambilRequestChat} telah di unban!"`,
                  mentions: [
                    ambilRequestChat.split("@")[1] + "@s.whatsapp.net",
                  ],
                },
            ...
            if (chat.includes("/listban")) {
              ...
              if (banPeople.length > 0) {
                ...
                const newTag = banPeople.map(
                  (e) => e.replace("@", " ") + "@s.whatsapp.net"
                );
                ...
                await sock.sendMessage(
                  number,
                  {
                    text: `Berikut adalah anggota yang di ban: ${list}`,
                    mentions: newTag,
                  },
                  { quoted: messages[0] },
                  2000
                ...
                await sock.sendMessage(number, {
                  text: "Tidak ada list ban untuk saat ini.",
                });
              }
            }
          } else if (chat.includes("/ban") || chat.includes("/unban")) {
            await sock.sendMessage(number, {
              text: "Maaf, fitur ini hanya tersedia di group",
            });
          }
          if (chat.includes("/nanya")) {
            await sock.sendMessage(
              number,
              { text: "Sebentar sedang mikir..." },
              { quoted: messages[0] },
              2000
            );
            await sock.sendMessage(
          ...
          if (chat === "Error") {
            await sock.sendMessage(number, {
              text: "Maaf, pesan yang saya tangkap error, silahkan kirim ulang!",
            });
          }
          if (
            (chat === "Disconnect" &&
              messages[0].key.participant == "6289612792131@s.whatsapp.net") ||
            (chat === "Disconnect" &&
              messages[0].key.remoteJid == "6289612792131@s.whatsapp.net")
          ) {
            await sock.sendMessage(number, {
              text: "Bot akan memutuskan koneksi dalam 5 detik.",
            });
            console.log("ya");
            setTimeout(() => {
              process.exit();
            ...
            await sock.sendMessage(
              number,
              { text: "Maaf, fitur disconnect tidak bisa anda gunakan." },
              { quoted: messages[0] }
            );
          }
        }
